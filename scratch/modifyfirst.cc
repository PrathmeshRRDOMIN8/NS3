...
main (int argc, char *argv[])
{
  ...
  ApplicationContainer serverApps = echoServer.Install (nodes.Get (0)); //will become 0
  serverApps.Start (Seconds (1.0));
  serverApps.Stop (Seconds (10.0));

  UdpEchoClientHelper echoClient (interfaces1.GetAddress (0), 9);  // interfaces1 last address which is the end node.
  echoClient.SetAttribute ("MaxPackets", UintegerValue (1));
  echoClient.SetAttribute ("Interval", TimeValue (Seconds (0.5)));
  echoClient.SetAttribute ("PacketSize", UintegerValue (1024));

  ApplicationContainer clientApps = echoClient.Install (nodes.Get (2)); //will become 2
