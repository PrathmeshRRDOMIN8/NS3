...
main (int argc, char *argv[])
{
  ...
  devices3 = P2P3.Install (nodes.Get (2), nodes.Get (3));
  devices4 = P2P4.Install (nodes.Get (2), nodes.Get (4));
  ...
  ApplicationContainer serverApps1 = echoServer1.Install (nodes.Get (4)); //will become 2
  serverApps1.Start (Seconds (1.0));
  serverApps1.Stop (Seconds (10.0));
  
  UdpEchoServerHelper echoServer2 (10);
  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
  ApplicationContainer serverApps2 = echoServer2.Install (nodes.Get (3)); //will become 2
  serverApps2.Start (Seconds (1.0));
  serverApps2.Stop (Seconds (10.0));

  UdpEchoClientHelper echoClient1 (interfaces4.GetAddress (1), 9);  // interfaces2 last address which is the end node.
  ...
  UdpEchoClientHelper echoClient2 (interfaces3.GetAddress (1), 10);  // interfaces2 last address which is the end node.
